...
const ELEVENLABS_VOICE_ID = process.env.ELEVENLABS_VOICE_ID || "8kBpuvY3rAhtsriFOB3c";
...
const AMAZON_CREATORS_CREDENTIAL_ID = process.env.AMAZON_CREATORS_CREDENTIAL_ID || "";
const AMAZON_CREATORS_SECRET = process.env.AMAZON_CREATORS_SECRET || "";
const AMAZON_CREATORS_VERSION = String(process.env.AMAZON_CREATORS_VERSION || "3.1").trim();
const AMAZON_CREATORS_MARKETPLACE = "www.amazon.ca";
const AMAZON_CREATORS_API_BASE = "https://creatorsapi.amazon";
const AMAZON_CREATORS_TOKEN_SKEW_MS = 60 * 1000;

let amazonCreatorsTokenCache = {
    accessToken: "",
    expiresAt: 0
};
...
function amazonCreatorsTokenEndpoint() {
    if (AMAZON_CREATORS_VERSION === "3.2") return "https://api.amazon.co.uk/auth/o2/token";
    if (AMAZON_CREATORS_VERSION === "3.3") return "https://api.amazon.co.jp/auth/o2/token";
    return "https://api.amazon.com/auth/o2/token";
}

async function getAmazonCreatorsAccessToken(forceRefresh = false) {
    if (!amazonCreatorsConfigured()) {
        const error = new Error("Amazon Creators API credentials are not configured.");
        error.code = "AMAZON_CREATORS_NOT_CONFIGURED";
        throw error;
    }
    ...
    if (
        !forceRefresh &&
        amazonCreatorsTokenCache.accessToken &&
        amazonCreatorsTokenCache.expiresAt - AMAZON_CREATORS_TOKEN_SKEW_MS > now
    ) {
    ...
    const response = await fetch(amazonCreatorsTokenEndpoint(), {
        ...
        body: JSON.stringify({
            ...
            client_id: AMAZON_CREATORS_CREDENTIAL_ID,
            client_secret: AMAZON_CREATORS_SECRET,
...
const AMAZON_CREATORS_SEARCH_INDEXES = new Set([
    "All",
    "Automotive",
    "Electronics",
    "GardenAndOutdoor",
...
function cleanAmazonSearchIndex(value) {
    const candidate = typeof value === "string" ? value.trim() : "";
    return AMAZON_CREATORS_SEARCH_INDEXES.has(candidate) ? candidate : "All";
}

function cleanAmazonCreatorsDetailUrl(value, asin = "") {
    if (typeof value !== "string" || !value.trim()) return amazonCanadaUrl(asin);
    try {
        const u = new URL(value);
        if (!u.hostname.toLowerCase().endsWith("amazon.ca")) return amazonCanadaUrl(asin);
        return u.toString();
    } catch {
        return amazonCanadaUrl(asin);
    }
}

async function searchAmazonCreators(query, searchIndex = "All", retryAuth = true) {
    const keywords = String(query || "").trim().slice(0, 180);
    if (!keywords) return [];
    ...
    if (response.status === 401 && retryAuth) {
        amazonCreatorsTokenCache = { accessToken: "", expiresAt: 0 };
        await getAmazonCreatorsAccessToken(true);
        return searchAmazonCreators(keywords, searchIndex, false);
    }

    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
        const apiError = Array.isArray(data?.errors) && data.errors.length ? data.errors[0] : null;
        const message =
            apiError?.message ||
            apiError?.code ||
            data?.message ||
...
function amazonCreatorsProductFromItem(item, candidate) {
    ...
    return {
        ...
        description:
            String(candidate?.description || "").trim() ||
            "A real Amazon.ca pick Chad found for this job.",
        asin,
        source_url: cleanAmazonCreatorsDetailUrl(item.detailPageURL, asin),
        retailer: "Amazon.ca",
        image_url: imageUrl,
        price
    };
...
async function resolveAmazonCreatorsProducts(candidates) {
    ...
    const cleaned = candidates
        ...
        .map(candidate => ({
            item_name: typeof candidate.item_name === "string" ? candidate.item_name.trim() : "",
            search_query: typeof candidate.search_query === "string" ? candidate.search_query.trim() : "",
            search_index: cleanAmazonSearchIndex(candidate.search_index),
            description: typeof candidate.description === "string" ? candidate.description.trim() : ""
        }))
        .filter(candidate => candidate.item_name && candidate.search_query)
        .slice(0, 4);
    ...
    const settled = await Promise.allSettled(
        cleaned.map(async candidate => {
            const items = await searchAmazonCreators(candidate.search_query, candidate.search_index);
            const product = items.map(item => amazonCreatorsProductFromItem(item, candidate)).find(Boolean);
            return product || null;
        })
    );
...
function isRecipeQuestion(message) {
    const text = String(message || "").toLowerCase().trim();
    if (!text) return false;

    const recipeSignals = [
        "recipe", "how do i make", "how to make", "how do i cook", "how to cook",
        "how do i bake", "how to bake", "ingredients for", "what ingredients",
        "make me a", "cook me a", "bake me a", "homemade"
    ];

    const foodSignals = [
        "chicken","beef","steak","pork","turkey","fish","salmon","shrimp","potato","potatoes",
        "pasta","spaghetti","pizza","bread","cake","cookies","soup","stew","chili","rice",
        "sauce","gravy","salad","coleslaw","burger","burgers","tacos","souvlaki","tzatziki",
        "roast","roasted","casserole","pie","pancakes","waffles","eggs","omelette","dessert"
    ];

    return recipeSignals.some(x => text.includes(x)) ||
        (["cook","bake","roast","grill"].some(x => text.includes(x)) &&
         foodSignals.some(x => text.includes(x)));
}
...
const SHOPPING_PROMPT = "You are Chad's job-prep assistant.\n\nThe user already asked Chad a DIY / repair / building question OR asked for a recipe/cooking instructions, and Chad has already answered it.\n\nNow Chad is building the useful shopping list automatically.\n\nCreate a concise, practical shopping/checklist for completing that exact job or recipe.\n\nVoice:\n- still Chad\n- mildly annoyed\n- funny\n- useful\n- no swearing\n- do not overdo the comedy\n\nThe list should include:\n- tools they realistically need\n- materials\n- consumables\n- optional helpful items only when genuinely useful\n- reasonable quantities when the question provides enough information\n- \"as needed\" or \"1\" when exact quantities cannot be known\n- keep the full list concise: usually 5 to 10 genuinely useful items\n- do NOT pad the list with obvious household clutter just to make it longer\n- combine closely related household cleanup items when appropriate\n\nRECIPE MODE:\n- If the question is a recipe or cooking request, make the list primarily the recipe ingredients/groceries.\n- Preserve the exact useful quantities from Chad's recipe answer. Every grocery item should carry the recipe amount when known.\n- If the recipe uses an ingredient in multiple stages, combine it into the total shopping quantity unless keeping the split is genuinely clearer.\n- Use type \"Grocery\" for food ingredients.\n- Add kitchen tools only when they are genuinely important to making the dish.\n- Fresh meat, eggs, dairy, ordinary produce and similar perishables belong in the shopping list, but they should NOT become Amazon product candidates.\n\nSafety:\n- do not turn a dangerous job into reckless instructions\n- if the answer indicates a professional is required, keep the list limited to safe diagnostic/prep items rather than equipment for unsafe work\n\nAMAZON CANDIDATES:\n- Do NOT invent ASINs, URLs, brand names, prices or Amazon products. The backend will query the real Amazon Creators API.\n- Instead, choose 2 to 4 shopping-list items that are strong online purchase opportunities and create a search candidate for each.\n- Good recipe candidates: specialty sauces, spices, oils, shelf-stable pantry ingredients, thermometers, strainers, cookware and genuinely useful kitchen tools.\n- Good DIY candidates: tools, testers, specialty consumables, hardware kits, accessories and job-specific gear.\n- Avoid ordinary fresh groceries and trivial household clutter.\n- Every candidate item_name MUST exactly match one item name in the items array.\n- search_query should be short and specific enough for Amazon.ca search, usually 2 to 7 words.\n- search_index must be one of: All, Automotive, Electronics, GardenAndOutdoor, GroceryAndGourmetFood, HealthPersonalCare, HomeAndKitchen, Industrial, OfficeProducts, SportsAndOutdoors, ToolsAndHomeImprovement.\n- description should briefly explain why that item is useful for this exact job.\n\nReturn only the structured response.";
    ...
    properties: {
        ...
        items: {
            ...
            items: {
                ...
                properties: {
                    ...
                    type: { type: "string", enum: ["Tool","Material","Consumable","Grocery","Optional"] },
                    note: { type: "string" }
                },
        ...
        products: {
            ...
            items: {
                ...
                required: ["item_name","search_query","search_index","description"]
            }
        }
    },
...
async function handleShoppingList(req, res) {
    try {
        ...
        const allowedTypes = new Set(["Tool","Material","Consumable","Grocery","Optional"]);
        ...
        const amazonCandidates = Array.isArray(decoded.products)
            ? decoded.products.filter(candidate => itemNames.has(String(candidate?.item_name || "").trim()))
            : [];

        const amazonResult = await resolveAmazonCreatorsProducts(amazonCandidates);
        const products = amazonResult.products;
        ...
        return res.json({
            ...
            items,
            products,
            amazon_picks_status: amazonResult.status,
...
function normalizeTtsComparable(value) {
    return String(value || "")
        .replace(/[*_`#>]/g, "")
        .replace(/\[(.*?)\]\((.*?)\)/g, "$1")
        .replace(/\s+/g, " ")
...
async function handleChadTts(req, res) {
    try {
        ...
        return res.status(500).json({
            success: false,
            error: "Chad's voice had a moment."
        });
    }
}
...
registerBoth("post", "/tts", handleChadTts);
